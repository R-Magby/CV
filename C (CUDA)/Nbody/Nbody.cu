...
__global__ void guardar_potencial(float *phi_n,float *phi_p1,int N, int t){
    ...
    if (idx<N && idy<N ){
        phi_p1[t*N*N +idy*N+idx] = (phi_n[idy*N+idx]);
...
int main(int argc, char *argv[]){
    ...
    phi_p1=(float *)malloc(size_time*N_grilla*N_grilla*sizeof(float));
    ...
    rellenar_rho(phi_p1,size_time*N_grilla*N_grilla);
    ...
    cudaMalloc((void **)&cuda_r, size_cuerpos);
    ...
    cudaMalloc((void **)&cuda_phi_p1,size_time*N_grilla*N_grilla*sizeof(float));
    ...
    cudaMemcpy(cuda_r, r, size_cuerpos, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(cuda_phi_p1, phi_p1, size_time*N_grilla*N_grilla*sizeof(float), cudaMemcpyHostToDevice);
   ...
   printf("%f \n", dt);
    ...
    for (int t=0;t<size_time;t++){
        ...
        guardar_potencial<<<grid,bloque>>>(cuda_phi_n,cuda_phi_p1,N_grilla,t);
        cudaDeviceSynchronize();
        }
        cudaMemcpy(r_tn,cuda_r_tn, size_time*size_cuerpos, cudaMemcpyDeviceToHost);


        cudaMemcpy(phi_p1,cuda_phi_p1,(size_time)*N_grilla*N_grilla*sizeof(float) , cudaMemcpyDeviceToHost);
        guardar_phi(phi_p1,(size_time)*N_grilla*N_grilla);
    ...
    free(r); free(dot_r); free(grilla); free(r_tn); free(phi); free(rho);

    cudaFree(cuda_r); cudaFree(cuda_dot_r); cudaFree(cuda_grilla);cudaFree(cuda_rho); cudaFree(cuda_err); cudaFree(cuda_grilla_G);
