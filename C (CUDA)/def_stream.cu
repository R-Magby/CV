...
__global__ void actualizacion(float *C, float *r, float *v, float *gravity ,int N_body,int N_g, float *G, float  step_grid, float m_p, float dt, int t){
...
int main(int argc, char *argv[]){
   ...
   // cudaMemcpy(cuda_rho, rho, N_grilla*N_grilla*sizeof(float), cudaMemcpyHostToDevice);
    ...
    for (int i=0;i<nstream;i++){
        int i_stream_particulas_tiempo=  i*2*N_b*size_time/nstream;
        int i_stream_particulas=  i*2*N_b/nstream;
        int i_stream_grilla=  i*N_grilla*N_grilla/nstream;


        cudaMemcpyAsync(&cuda_rho[i_stream_grilla], &rho[i_stream_grilla], N_grilla*N_grilla*sizeof(float)/nstream, cudaMemcpyHostToDevice, stream[i]);
        cudaMemcpyAsync(&cuda_phi_n[i_stream_grilla], &phi[i_stream_grilla], N_grilla*N_grilla*sizeof(float)/nstream, cudaMemcpyHostToDevice, stream[i]);
   ...
   dim3 grid((int)ceil((float)(N_grilla)/(thread*nstream2d)),(int)ceil((float)(N_grilla)/(thread*nstream2d)));

   
   printf("%f \n", dt);


        for (int t=0;t<size_time/nstream;t++){
            if (t%100==0){
                printf("%d \n",t);
            }
            ...
            potencial<<<grid,bloque>>>(&cuda_phi_n[i_stream_grilla],&cuda_rho[i_stream_grilla],cuda_err,N_grilla/(nstream/2),(float) L_size/N_grilla,100000);
        ...
        for (int i=0;i<nstream;i++){
            ...
            cudaMemcpyAsync(&rho[i_stream_grilla], &cuda_rho[i_stream_grilla], N_grilla*N_grilla*sizeof(float)/nstream, cudaMemcpyDeviceToHost, stream[i]);
            cudaMemcpyAsync(&phi[i_stream_grilla], &cuda_phi_n[i_stream_grilla], N_grilla*N_grilla*sizeof(float)/nstream, cudaMemcpyDeviceToHost, stream[i]);

            cudaMemcpyAsync(&r_tn[i_stream_particulas_tiempo], &cuda_r_tn[i_stream_particulas_tiempo], size_time*2*N_b*sizeof(float)/nstream, cudaMemcpyDeviceToHost, stream[i]);
