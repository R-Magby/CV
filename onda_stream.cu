...
__global__ void suma_matriz(float *A,float *B,float *C,int Nx, int Ny,int T){
	int idx=threadIdx.x + blockDim.x*blockIdx.x;
	int idy=threadIdx.y + blockDim.y*blockIdx.y;
	
    for (int t=0;t<T;t++){
    	    C[t*Nx*Ny+idy*Nx+idx] = A[t*Nx*Ny+idy*Nx+idx]+B[t*Nx*Ny+idy*Nx+idx];
...
void guardar_salida(float *data,int Nx, int Ny,int T) {
    ...
    FILE *fp = fopen("onda_2d_stream.dat", "w");
...
cudaStreamCreate ( &stream2) ;
    ...
    // Copiar al device
    cudaMemcpy(d_u_s1, u_s1, size*T, cudaMemcpyHostToDevice);
    cudaMemcpy(d_u_m1_s1, u_m1_s1, size, cudaMemcpyHostToDevice);

    cudaMemcpy(d_u_s2, u_s2, size*T, cudaMemcpyHostToDevice);
    cudaMemcpy(d_u_m1_s2, u_m1_s2, size, cudaMemcpyHostToDevice);


    // Invocar kernel con N bloques de 1 thread cada uno
    dim3 bloque (32,32);
    dim3 grid(ceil(Nx/32),ceil(Ny/32));
   ...
   cudaError_t err = cudaGetLastError();
	...
	float *phi_final=(float *)malloc(size*T);
    float *cuphi_final;

    cudaMalloc((void **)&cuphi_final, size*T);

    suma_matriz<<<grid,bloque>>>(d_u_s1,d_u_s2,cuphi_final,Nx,Ny,T);

    cudaMemcpy(phi_final, cuphi_final, size*T, cudaMemcpyDeviceToHost);
    ...
    free(phi_final);cudaFree(cuphi_final);
	return 0;
}
